...
static char FAR C[28];
static char FAR D[28];
static char FAR KS[768];

static char FAR L[64];
static char FAR tempL[32];
static char FAR preS[48];
static char FAR f[32];

static char FAR KeyBuilder[64];
...
void des_cipher(unsigned char FAR *block, int crypt_mode)
{
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        for (j = 0; j < 8; j++)
        {
            ...
            f[0] = (S[j * 4 + SIndex] >> 3) & 0x01;
            f[1] = (S[j * 4 + SIndex] >> 2) & 0x01;
            f[2] = (S[j * 4 + SIndex] >> 1) & 0x01;
            f[3] =  S[j * 4 + SIndex]       & 0x01;
        }
...
void InitNormalKey(const char FAR *Key)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        //
        // Expand the current byte value into indiviudal bits
        //
